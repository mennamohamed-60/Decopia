...
const fetchDocumentation = async () => {
  ...
  const res = await fetch(
    "https://decopia-management-system.runasp.net/api/documentation",
...
export default function DocumentationFiles() {
  ...
  const totalPages = Math.ceil(docs.length / ROWS_PER_PAGE);
  const currentRows = docs.slice(
  ...
  const handleDownload = async (publicId, fileName) => {
    ...
    try {
      const res = await fetch(
        `https://decopia-management-system.runasp.net/api/documentation/${publicId}/download`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="pt-10">
      <section className="bg-gray-900 container m-auto rounded-lg shadow-md p-6">
        <h2 className="text-2xl font-bold text-white mb-2">API Documentation Files</h2>
        ...
        <div>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-gray-500">
              ...
              <p className="text-sm">No documentation files uploaded yet.</p>
            </div>
          ) : (
            currentRows.map((doc) => (
              <div
              ...
              >
                ...
                <div className="flex items-center gap-5 w-2/5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h2 className="font-bold text-white truncate">{doc.fileName}</h2>
                    <p className="text-gray-400 text-sm">{formatDate(doc.uploadedAt)}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between w-3/5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-white">{doc.uploadedBy}</p>
                    </div>
                  </div>

                  <div>
                    <p className="text-xs text-gray-400">Size</p>
                    <p className="text-sm font-semibold text-white">{doc.fileSizeFormatted}</p>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      {doc.hasNotes ? "Available" : "None"}
                    </span>
                  </div>

                  <button
                    onClick={() => handleDownload(doc.publicId, doc.fileName)}
                    disabled={downloadingId === doc.publicId}
