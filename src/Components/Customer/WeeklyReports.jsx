...
const fetchCustomerFiles = async () => {
  ...
  const res = await fetch(
    "https://decopia-management-system.runasp.net/api/customer/files",
...
export default function WeeklyReports() {
  ...
  const { data: files = [], isLoading, error } = useQuery({
    queryKey: ["customer-files"],
    queryFn: fetchCustomerFiles,
  ...
  const handleDownload = async (publicId, fileName) => {
    ...
    try {
      const res = await fetch(
        `https://decopia-management-system.runasp.net/api/customer/files/${publicId}/download`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="">
      <section className="bg-gray-900 container m-auto rounded-lg shadow-md p-4">
        <h2 className="text-2xl font-bold text-white mb-2">Weekly Reports</h2>
          ...
          {isLoading ? (
            ...
            currentRows.map((file) => (
              ...
              >
                ...
                {/* Right: uploader + size + notes + download */}
                <div className="flex items-center justify-between w-3/5">
                  <div className="flex items-center gap-3">
