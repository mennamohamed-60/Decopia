...
const BACKEND_URL = "https://decopia-management-system.runasp.net/api/slack/alerts";
...
function TableSkeleton({ rows = 8 }) {
  return Array.from({ length: rows }).map((_, i) => (
    ...
    >
      ...
      <td className="px-4 py-3"><div className="h-4 w-40 rounded bg-gray-700" /></td>
      <td className="px-4 py-3"><div className="h-4 w-28 rounded bg-gray-700" /></td>
      <td className="px-4 py-3"><div className="h-4 w-20 rounded bg-gray-700" /></td>
      <td className="px-4 py-3"><div className="h-7 w-24 rounded-lg bg-gray-700" /></td>
      <td className="px-4 py-3 text-center"><div className="h-6 w-10 rounded-full bg-gray-700 mx-auto" /></td>
      <td className="px-4 py-3"><div className="h-4 w-32 rounded bg-gray-700" /></td>
    </tr>
  ));
}
      ...
      >
        ...
        <div className="flex flex-col gap-3 mb-5">
          <div className="flex items-center justify-between rounded-lg border border-[#2e4a48] bg-gray-800 px-4 py-2.5">
            ...
            <span className="text-sm text-gray-100 font-semibold">{log.attackType}</span>
          </div>
          <div className="flex items-center justify-between rounded-lg border border-[#2e4a48] bg-gray-800 px-4 py-2.5">
          ...
          <div className="flex items-center justify-between rounded-lg border border-[#2e4a48] bg-gray-800 px-4 py-2.5">
            ...
            <span
              ...
              style={{
                background: parseInt(log.newAttacks) > 10 ? "rgba(226,75,74,0.15)" : "rgba(20,184,166,0.1)",
                color: parseInt(log.newAttacks) > 10 ? "#e24b4a" : "#14b8a6",
              }}
            >
              {log.newAttacks}
            </span>
...
export default function WazuhDashboard() {
  ...
  const filtered = logs.filter((log) => {
    const matchSearch =
      search === "" ||
      log.attackType?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const totalAttacksSum = logs.reduce((sum, l) => sum + parseInt(l.newAttacks || 0), 0);
